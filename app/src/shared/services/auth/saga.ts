...
import { saveUserMessage, UserMessageStatus } from 'features/user-message';
...
export function* loginToGame(
  ...
  if (res.socketId) {
    yield put(saveUserMessage({ status: UserMessageStatus.success, message: res.message }));
    yield put(saveUsername({ user: res.user, socketId: res.socketId }));
    payload.navigate('/dashboard');
  } else {
    yield put(saveUserMessage({ status: UserMessageStatus.error, message: res.message }));
  }
}
